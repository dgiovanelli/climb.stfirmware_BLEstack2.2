...
static bStatus_t gattServApp_SendNotiInd( uint16 connHandle, uint8 cccValue,
                                          uint8 authenticated, gattAttribute_t *pAttr, uint8 notiLength,
 ...
 * @param   len - the length of data to be sent (0 < len <= MAX_MTU_SIZE-3)
 ...
 */
bStatus_t GATTServApp_ProcessCharCfg_giova( gattCharCfg_t *charCfgTbl, uint8 *pValue, uint8 len,
                                      ...
                                      pfnGATTReadAttrCB_t pfnReadAttrCB )
{
  ...
  for ( i = 0; i < linkDBNumConns; i++ )
  {
         ...
         ( pItem->value != GATT_CFG_NO_OPERATION ) )
    {
      ...
      // Find the characteristic value attribute (pValue will equal to pAttr->pValue)
      ...
      if ( pAttr != NULL )
      {
        if ( pItem->value & GATT_CLIENT_CFG_NOTIFY )
        {
           status |= gattServApp_SendNotiInd( pItem->connHandle, GATT_CLIENT_CFG_NOTIFY, 
                                              authenticated, pAttr, len, taskId, pfnReadAttrCB );
        }
 ...
 * @param   notiLength - the length of data to be sent (0 < len <= MAX_MTU_SIZE-3)
 * @param   taskId - task to be notified of confirmation.
 * @param   pfnReadAttrCB - read callback function pointer.
 *
 * @return  Success or Failure
